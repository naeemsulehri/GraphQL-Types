...
import { readLocalFile, readFileContent } from "../../utils/file";
...
const ImportForm = ({ models }) => {
  ...
  const upload = () => {
    ...
    setLoading(true);
    const fileType = sourceFile.name.split(".").pop();
    switch (fileType.toLowerCase()) {
      case "json":
        readLocalFile(sourceFile, JSON.parse)
          ...
          .catch((error) => {
            strapi.notification.error(
              "Something wrong when uploading the JSON file, please check the file and try again."
            );
            console.error(error);
          })
          .finally(() => {
        ...
        break;
      case "csv":
        readFileContent(sourceFile).then(setSource).catch((error) => {
          strapi.notification.error("Something wrong when uploading the CSV file, please check the file and try again.");
          console.error(error);
        }).finally(() => {
          setLoading(false);
        });
